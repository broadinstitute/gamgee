...
class IndexedSamReader {
  public:
    ...
    IndexedSamReader(const std::string& filename, const std::vector<std::string>& interval_list) :
      ...
      m_sam_index_ptr {sam_index_load(m_sam_file_ptr, (filename + ".bai").c_str())}, // TODO: Not checking for errors.
    ...
    ~IndexedSamReader() {
      ...
      if (m_sam_index_ptr)
        hts_idx_destroy(m_sam_idx_ptr); // TODO: Copied from samtools. Is there a {bam,sam}_idx_destroy alias? Didn't find under "BAM/CRAM indexing" in sam.h
    }
